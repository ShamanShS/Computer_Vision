...
def brief(image, keypoint, pairs, keypoint_orientations):
    x, y = keypoint
    descriptor = np.zeros(len(pairs[0]), dtype=np.uint8)
    ind = np.where((filtered_keypoints1 == keypoint).all(axis=1))[0][0]
...
if __name__ == "__main__":
    ...
    gray_image = np.mean(image_mas, axis=2).astype(np.uint8)
    ...
    for key in filtered_keypoints1:
        descriptor = brief(blur_img, key, orientation_pairs, keypoint_orientations)
    ...
    plt.axis('off')
    plt.show()
    np.savetxt('descriptors.txt', descriptors, fmt='%d', delimiter=',')